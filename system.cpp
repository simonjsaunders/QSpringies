    ...
    *is_mass = true;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (m.status & S_ALIVE)
        {
            ...
            if ((dist = square(m.x - (double)x) + square(m.y - (double)y) - (double)square(radius)) < min_dist)
            {
